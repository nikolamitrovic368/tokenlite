...
class UserPanel
{
    ...
    public static function user_token_block($data = '', $atttr = '')
    {
        ...
        $vip_badge = '';
        if ($stage->vip_only) {
            $vip_badge = '<span class="badge badge-danger">Reserved for Partners</span>&nbsp;&nbsp;&nbsp;&nbsp;<a href="mailto:contact@bullnbear.finance" class="link">Contact Us</a><div class="gaps-1x"></div>';
        }
        ...
        if ($vers =='buy') {
            ...
            $return .= '<h6 class="card-title card-title-sm">'. $stage->name .'<span class="badge badge-success ucap">' . __($status) . '</span></h6>';
            $return .= $vip_badge;
     ...
     * @return void
     */
    public static function add_whitelist_alert()
    {
        return '<a href="javascript:void(0)" class="btn btn-danger btn-xl btn-between w-100 mgb-1-5x">' . __('This stage is limited to our partners') . ' </a>
        <div class="gaps-1x mgb-0-5x d-lg-none d-none d-sm-block"></div>';
    }
    ...
    public static function user_account_wallet($data = null, $atttr = '')
    {
        ...
        if ($user->walletAddress) {
            $whitelist = Whitelist::where(['address' => $user->walletAddress])->get();
            if ($whitelist->count()) {
                $badge .= '<span class="badge badge-success" style="margin-top: -4px">Whitelisted</span>';
            }
        }

        $uwallet = '<h6 class="card-title' . $title_cls . '">' . __('Receiving Wallet') . $badge . '</h6><div class="gaps-1x"></div>';
