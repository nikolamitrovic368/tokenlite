...
class PaymentMethod extends Model
{
    ...
    public static function getLiveRatesCoingecko($base = null) {
        $coingeckoAPI = "https://api.coingecko.com/api/v3/simple/price?ids=tron,tether,binancecoin,usd-coin,binance-usd,bitcoin,ethereum,litecoin&vs_currencies=usd";
        
        $cl = new Client();
        $rates = [];
        try {
            $response = $cl->request('GET', $coingeckoAPI);
            if ($response->getStatusCode() == 200) {
                $result = json_decode($response->getBody(), true);
                $rates = [
                    'currencies' => [
                        'ETH' => 1 / data_get($result, 'ethereum.usd'),
                        'BTC' => 1 / data_get($result, 'bitcoin.usd'),
                        'LTC' => 1 / data_get($result, 'litecoin.usd'),
                        'BNB' => 1 / data_get($result, 'binancecoin.usd'),
                        'USDT' => 1 / data_get($result, 'tether.usd'),
                        'TRX' => 1 / data_get($result, 'tron.usd'),
                        'USDC' => 1 / data_get($result, 'usd-coin.usd'),
                        'BUSD' => 1 / data_get($result, 'binance-usd.usd'),
                        base_currency(true) => 1,
                    ]
                ];
                Setting::updateOrCreate(['field' => 'exratesapi_error_msg'], ['value' => '']);
            } else {
                throw new \Exception('Coingecko response status failed.');
            }
        } catch (\Exception $e) {
    ...
    {
        if ($force === true) {
            return self::getLiveRatesCoingecko($base);
        }
